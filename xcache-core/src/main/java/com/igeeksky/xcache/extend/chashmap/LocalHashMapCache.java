 ...
 */
public class LocalHashMapCache implements Cache {
	
	private Logger logger = LoggerFactory.getLogger(LocalHashMapCache.class);
	...
	private final LocalHashMapCacheCleanner cleanner;
	
	private volatile boolean isCleaning = false;
	
	LocalHashMapCache(CacheKey cacheKey, int maxElementsNum, ScheduledExecutorService executor){
		this(cacheKey.getCacheName(), cacheKey.getAliveTime(), maxElementsNum, executor);
	}
	
	LocalHashMapCache(String name, long expiration, int maxElementsNum, ScheduledExecutorService executor) {
		this.name = name;
		...
		this.cleanner = new LocalHashMapCacheCleanner(this);
	}
	...
	private LocalValueWrapper toValueWrapper(Object value) {
		return new LocalValueWrapper(TimeGen.INSTANCE.currSecond()+expiration, value);
	}
		...
		while(it.hasNext()){
			...
			if(null != entry && null != (wrapper = entry.getValue())){
				...
				if(null != key){
					VisitRecord record = timeStore.get(key);
					if(null == record){
						if((expireTime - expiration + perridTime) < currentTime){
							it.remove();
	...
	private static class VisitRecord{
		...
		public long lastTime = TimeGen.INSTANCE.currSecond();
		
		public int visitTimes = 1;

		public VisitRecord update(){
			this.lastTime = TimeGen.INSTANCE.currSecond();
			visitTimes++;
			return this;
		}
	}
